...
class LineCounter:
    ...
    def count_lines(self, file_path: Path, language: Language) -> Tuple[int, int, int]:
        ...
        single_line_comment, multi_line_start, multi_line_end = self._get_comment_patterns(language)
        ...
        for line in lines:
            ...
            if multi_line_start and multi_line_end:
                if not in_multiline_comment and stripped.startswith(multi_line_start):
                    comment += 1
                    in_multiline_comment = not stripped.endswith(multi_line_end) or \
                                         stripped.count(multi_line_start) > stripped.count(multi_line_end)
                    continue
