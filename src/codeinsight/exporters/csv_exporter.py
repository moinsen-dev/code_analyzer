...
class CSVExporter:
    ...
    def export(self, report: AnalysisReport, output_path: Path):
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            # Write header
            writer.writerow([
                'File Path', 'Relative Path', 'Language', 'Lines of Code', 
                'Blank Lines', 'Comment Lines', 'Size (bytes)', 'Last Modified',
                'Cyclomatic Complexity', 'Cognitive Complexity', 
                'Maintainability Index', 'Technical Debt Ratio'
            ])
            ...
            for file_insight in report.top_files:
                ...
                row = [
                    ...
                    file_metrics.language.value if hasattr(file_metrics.language, 'value') else str(file_metrics.language),
                    file_metrics.lines_of_code,
                    file_metrics.blank_lines,
                    file_metrics.comment_lines,
                    file_metrics.size_bytes,
                    file_metrics.last_modified.isoformat() if file_metrics.last_modified else '',
                ]
