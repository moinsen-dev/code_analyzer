...
""")
        ...
        print("Languages:", {lang.value: count for lang, count in report.language_distribution.items()})
        
        # Check that .git files are not in the results
        git_files = [f for f in report.top_files if '.git' in f.file_metrics.relative_path]
        assert len(git_files) == 0, f"Found .git files in analysis: {[f.file_metrics.relative_path for f in git_files]}"
        
        # Check that image files are not in the results
        image_files = [f for f in report.top_files if '.png' in f.file_metrics.relative_path]
        assert len(image_files) == 0, f"Found image files in analysis: {[f.file_metrics.relative_path for f in image_files]}"
        
        # Check that node_modules files are not in the results
        node_files = [f for f in report.top_files if 'node_modules' in f.file_metrics.relative_path]
        assert len(node_files) == 0, f"Found node_modules files in analysis: {[f.file_metrics.relative_path for f in node_files]}"
        
        # Check that source files are included
        source_files = [f for f in report.top_files if f.file_metrics.relative_path in ['main.dart', 'pubspec.yaml', 'pubspec.lock']]
        assert len(source_files) >= 2, f"Source files not properly included: {[f.file_metrics.relative_path for f in source_files]}"
        
        print("Complex filtering test passed!")
        print("Top files by line count:")
        for file_insight in report.top_files[:5]:
            print(f"  {file_insight.file_metrics.relative_path}: {file_insight.file_metrics.lines_of_code} lines")
