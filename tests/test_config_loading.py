...
def test_configuration_loading():
    """Test that local .codeinsight.yml is properly loaded and used"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config_path = temp_path / ".codeinsight.yml"
...
""")
        ...
        ignored_files = [f for f in report.top_files if 'test.custom.py' in f.file_metrics.relative_path]
        assert len(ignored_files) == 0, "Custom ignored files should not appear in analysis"
        
        print("Configuration loading and usage test passed!")
        print(f"Loaded config version: {config_manager.config.version}")
        print(f"Python max line length: {config_manager.config.languages['python'].max_line_length}")
