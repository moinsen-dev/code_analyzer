...
""")
        ...
        generated_files = [f for f in report.top_files if '.g.dart' in f.file_metrics.relative_path]
        assert len(generated_files) == 0, f"Found generated files in analysis: {[f.file_metrics.relative_path for f in generated_files]}"
        
        # Check that lock files are not in the results
        lock_files = [f for f in report.top_files if 'pubspec.lock' in f.file_metrics.relative_path]
        ...
        project_files = [f for f in report.top_files if '.pbxproj' in f.file_metrics.relative_path or '.xib' in f.file_metrics.relative_path]
        assert len(project_files) == 0, f"Found project files in analysis: {[f.file_metrics.relative_path for f in project_files]}"
        
        # Check that source files are included
        source_files = [f for f in report.top_files if f.file_metrics.relative_path in ['main.dart', 'task.dart']]
