...
""")
        ...
        lock_files = [f for f in report.top_files if '.lock' in f.file_metrics.relative_path]
        ...
        json_files = [f for f in report.top_files if f.file_metrics.relative_path.endswith('.json') and f.file_metrics.relative_path != 'package.json']
        assert len(json_files) == 0, f"Found regular JSON files in analysis: {[f.file_metrics.relative_path for f in json_files]}"
        
        # Check that package.json is still included
        package_json_files = [f for f in report.top_files if f.file_metrics.relative_path == 'package.json']
        assert len(package_json_files) == 1, "package.json should be included in analysis"
        
        # Check that source files are included
        source_files = [f for f in report.top_files if f.file_metrics.relative_path in ['main.dart']]
