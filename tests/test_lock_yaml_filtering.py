...
""")
        ...
        lock_yaml_files = [f for f in report.top_files if 'lock.yaml' in f.file_metrics.relative_path]
        assert len(lock_yaml_files) == 0, f"Found lock.yaml files in analysis: {[f.file_metrics.relative_path for f in lock_yaml_files]}"
        
        # Check that regular YAML files are still included
        yaml_files = [f for f in report.top_files if f.file_metrics.relative_path.endswith(('.yaml', '.yml')) and 'lock.yaml' not in f.file_metrics.relative_path]
        assert len(yaml_files) >= 2, f"Regular YAML files not properly included: {[f.file_metrics.relative_path for f in yaml_files]}"
        
        # Check that source files are included
        source_files = [f for f in report.top_files if f.file_metrics.relative_path == 'main.py']
        assert len(source_files) >= 1, f"Source files not properly included: {[f.file_metrics.relative_path for f in source_files]}"
