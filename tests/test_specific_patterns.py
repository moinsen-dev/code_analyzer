...
""")
        ...
        freezed_files = [f for f in report.top_files if '.freezed.dart' in f.file_metrics.relative_path]
        assert len(freezed_files) == 0, f"Found freezed files in analysis: {[f.file_metrics.relative_path for f in freezed_files]}"
        
        # Check that specific lock files are not in the results
        lock_files = [f for f in report.top_files if '-lock.json' in f.file_metrics.relative_path]
        assert len(lock_files) == 0, f"Found lock files in analysis: {[f.file_metrics.relative_path for f in lock_files]}"
        
        # Check that source files are included
        source_files = [f for f in report.top_files if f.file_metrics.relative_path in ['main.dart', 'user.dart']]
